...
class DistanceFilterViewHelper extends \TYPO3\CMS\Fluid\Core\ViewHelper\AbstractViewHelper{
	...
	public function render($keyword, $defaultDistance = '', $distanceFilter='') {
		...
		$linkUrl = $helper->getFullDomain().'/?id='.$configuration['search.']['targetPageId'].
			'&tx_solrgeo_search[q]='.$keyword;
		if(!empty($ranges) && $keyword != "") {
			$distanceFilterBox = '
			<div class="secondaryContentSection">
				<div class="csc-header">
					<h3 class="csc-firstHeader">'.
						$GLOBALS['LANG']->sL('LLL:EXT:solrgeo/Resources/Private/Language/locallang_search.xml:tx_solrgeo.filter.distance').
					'</h3>
				</div>';
			...
			if($showRemoveDistanceFilter) {
				$distanceFilterBox .= '<a href="'.$linkUrl.'">'.
					$GLOBALS['LANG']->sL('LLL:EXT:solrgeo/Resources/Private/Language/locallang_search.xml:tx_solrgeo.faceting_removeFilter').
					'</a>';
			}
